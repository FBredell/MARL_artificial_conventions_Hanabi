...
@gin.configurable
def create_agent(environment, obs_stacker, convention_encoder, agent_type='DQN'):
  ...
  if agent_type == 'DQN':
    return dqn_agent.DQNAgent(observation_size=obs_stacker.observation_size(),
                              num_actions=convention_encoder.convention_action_space,
...
def run_one_episode(agent, environment, obs_stacker, summary_writer, convention_encoder):
  ...
  """
  global global_score_per_episode
  ...
  convention_encoder.reset()
  current_player, legal_moves, observation_vector = (
        parse_observations(observations, environment.num_moves(), obs_stacker))
  
  convention_legal_moves = convention_encoder.available_conventions(environment)  #basic implementation
  # if global_episode_counter < 1000000: convention_legal_moves = convention_encoder.available_conventions(environment) #transfer learning
  # else: convention_legal_moves = convention_encoder.available_conventions(environment, disable_encoding = True) #transfer learning

  combined_legal_moves = format_legal_moves(convention_legal_moves, convention_encoder.convention_action_space)

  action = agent.begin_episode(current_player, combined_legal_moves, observation_vector)
  env_action = convention_encoder.encode_action(action.item(), environment)
  ...
  # Keep track of per-player reward, and also to use with forward accumulated reward in turn-based setting
  reward_since_last_action = np.zeros(environment.players)

  while not is_done:
    # print(f"Game state: {environment.state}")
    # print(f"environment legal moves: {legal_moves}")
    # print(f"convention legal moves: {convention_legal_moves}")  #for debugging
    # print(f"agent action: {action}")
    # print(f"env action: {env_action}")

    observations, reward, is_done, _ = environment.step(env_action)
    # print(f"environment state: {environment.state}")
    # print(f"player_hands: {environment.state.player_hands()}")
    ...
    if reward == 0: modified_reward -= 0.01   #slight negative per turn, found it helps slightly

    #---------------test for forward accumulated reward discount gamma--------------------------
    discount_counter = 0
    # player_turn = current_player + 1 if current_player+1 < environment.players else 0
    player_turn = current_player
    for i in  range(environment.players):
      reward_since_last_action[player_turn] += modified_reward*(ccr_gamma**discount_counter)
      discount_counter += 1
      # player_turn = player_turn + 1 if player_turn+1 < environment.players else 0
      player_turn = player_turn - 1 if player_turn > 0 else environment.players-1
    #--------------------------------------------------------------------------------------------
    ...
    if current_player in has_played:
      action = agent.step(reward_since_last_action[current_player],
                          current_player, combined_legal_moves, observation_vector)
      env_action = convention_encoder.encode_action(action.item(), environment)
    else:
      # Each player begins the episode on their first turn (which may not be
      # the first move of the game).
      action = agent.begin_episode(current_player, combined_legal_moves,
                                  observation_vector)
      env_action = convention_encoder.encode_action(action.item(), environment)                            
  ...
  if not agent.eval_mode:  
    global_score_per_episode.append(score)
    global_episode_counter += 1
    # summary_writer.add_event("reward_per_eps", episode_return, global_reward_per_episode)
    # tf.summary.scalar("lr", episode_return)
    summary_writer.add_scalar("score/eps", score, global_episode_counter)
...
def run_one_phase(agent, environment, obs_stacker, min_steps, statistics,
                  run_mode_str, summary_writer, convention_encoder):
  ...
  num_episodes = 0
  sum_returns = 0.
  # global global_reward_per_episode
  # global global_episode_counter

  while step_count < min_steps:
    episode_length, episode_return = run_one_episode(agent, environment,
                                                     obs_stacker, summary_writer, convention_encoder)
    statistics.append({
    ...
    # global_reward_per_episode.append(episode_return)
    # global_episode_counter += 1
    # # summary_writer.add_event("reward_per_eps", episode_return, global_reward_per_episode)
    # # tf.summary.scalar("lr", episode_return)
    # summary_writer.add_scalar("reward/eps", episode_return, global_episode_counter)
...
def run_one_iteration(agent, environment, obs_stacker,
                      iteration, training_steps, summary_writer, convention_encoder,
  ...
  number_steps, sum_returns, num_episodes = (
      run_one_phase(agent, environment, obs_stacker, training_steps, statistics,
                    'train', summary_writer, convention_encoder))
  ...
  if evaluate_every_n is not None and iteration % evaluate_every_n == 0:
    ...
    for _ in range(num_evaluation_games):
      episode_data.append(run_one_episode(agent, environment, obs_stacker, summary_writer, convention_encoder))
...
def run_experiment(agent,
                   ...
                   checkpoint_dir, convention_encoder,
                   num_iterations=200,
                   training_steps=5000,
  ...
  writer = SummaryWriter(log_dir="logs/full_hanabi_3p/rainbow_convention_encouded_official_3p_non_lenient_"+current_time)
  df = pd.DataFrame()
  ...
  for iteration in range(start_iteration, num_iterations):
    ...
    statistics = run_one_iteration(agent, environment, obs_stacker, iteration,
                                   training_steps, writer, convention_encoder)
  ...
  df[0] = global_score_per_episode
  df.to_csv(f"data/rainbow_full_hanabi_encouded_official_3p_non_lenient_{current_time}.csv") 